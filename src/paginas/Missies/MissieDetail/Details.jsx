...
const Details = ({
  ...
  const {
    ...
    queryFn: async () => {
      const response = await axiosPrivate.get(
        `${axiosUrls("GetMissieDetail")}/${missieid}`
      );
      console.log(response.data);
      setStartdatum(response.data.startDatum);
      setEinddatum(response.data.eindDatum);
      setMissieDetail(response.data);
      setMissieAfgesloten(response.data.afgesloten);
      setReadOnly(() => {
        return !isOrganisator || missieAfgesloten ? true : false;
      });
  ...
  const updateMutation = useMutation({
    mutationFn: async (detail) => {
      return axiosPrivate.post(axiosUrls("PostMissieDetail"), detail);
    },
    onSuccess: (data) => {
            ...
            <Form className="pt-1" id="formMissieDetail">
                ...
                <Col md={10} sm={12}>
                  <Form.Control
                    ...
                    onChange={(e) =>
                      setMissieDetail({
                        ...missieDetail,
                        startDatum: new Date(e.target.value).toISOString(),
                        eindDatum: new Date(e.target.value).toISOString(),
                      })
                    }
                    required
                  ></Form.Control>
              ...
              {isOrganisator ? (
                <Row>
                  ...
                  ) : !missieDetail?.afgesloten ? (
                    <Col sm={12} md={{ span: 10, offset: 2 }}>
                      <Button variant="success" onClick={BewaarDetails}>
                        <FaFloppyDisk /> Bewaar
                      </Button>
                    </Col>
                  ) : (
